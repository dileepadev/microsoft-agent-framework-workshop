"""
Configuration — the four variables that decide which model your exercises talk to.

This is the same contract `app/` uses, ported here because `lab/` is its own
`uv` project and shares nothing with `app/`. The whole workshop rests on one
idea: swapping the model should be a config change, not a rewrite. That promise
only holds if configuration is small, so the entire contract is four variables:
...
Two rules this module exists to enforce:
  ...
  2. Missing configuration fails at startup, naming the variable to set. You
     should never have to read a stack trace to learn you forgot to paste an
     API key into an exercise.
"""
...
# root, because lab/ is its own environment.
...
def normalise_provider(value: str) -> str:
    ...
    Accepts `Azure_OpenAI`, `azure-openai` and `AZURE-OPENAI` alike, so a typo
    that was never really an error loses you no time.
    """
...
def load_env(path: str | Path | None = None, *, override: bool = False) -> Path | None:
    ...
    Real environment variables win over the file unless `override=True`. That
    is what you want on a deployed host, where the platform injects
    configuration and there is no `.env` at all.
    """
...
class Settings:
    """The resolved provider configuration for a single exercise."""
    ...
    def from_env(cls, *, load: bool = True) -> Settings:
        ...
        if not provider:
            ...
            raise ConfigError(
                ...
                f"Copy lab/.env.example to lab/.env and set {PROVIDER_VAR} to the "
