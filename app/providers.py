...
GitHub Models was retired on 30 July 2026 and took the previous edition of this
workshop with it, because that edition had one provider welded into the agent.
This module is the answer to that: the agent code never names a vendor. It asks
for a chat client, and this factory decides which one to build from
`LLM_PROVIDER`.

    from providers import create_chat_client
    client = create_chat_client()          # reads .env

Everything downstream — `agent.py`, tools, MCP, sessions, workflows — is written
once against that client and never changes when the provider does.

Adding a provider means adding one `ProviderSpec` below. Nothing else moves.
...
class ProviderSpec:
    ...
    client: str
    package: str
    #: The `app` optional-dependency extra that installs `package`, or None when
    #: it is already a base dependency.
    extra: str | None
...
def get_spec(key: str) -> ProviderSpec:
    ...
    raise ConfigError(
        ...
        f"Supported values:\n{listing}\n\n"
        f"Set {PROVIDER_VAR} in app/.env to one of the values above."
    )
...
def _check_configured(spec: ProviderSpec, settings: Settings) -> None:
    ...
    message = (
        ...
        f"Set the following in app/.env:\n{listing}"
    )
    if already:
...
def load_client_class(spec: ProviderSpec) -> type[Any]:
    ...
    run the workshop. Importing every vendor SDK at module load would make the
    ...
    and the participant is left with the framework's own advice to run `pip
    install`, which is the wrong command for this uv project.
    """
    try:
...
def create_chat_client(settings: Settings | None = None) -> Any:
    ...
    This is the only function in the project that knows a vendor exists.
